...
| No `init()` for registration | Explicit registration in the dispatcher is traceable; `init()` execution order is implicit |
...
- `ff.ErrHelp` treated as success by `main`; `ff.ErrNoExec` absorbed by the dispatcher
...
└── cmd/
    └── root/                      # Default name; configurable via climax init --root-pkg
        └── root.go                # root.Config: shared I/O, flags, root ff.Command
```
...
// Version is set at build time:
// go build -ldflags "-X '<org>/<repo>/cmd/version.Version=1.2.3'"
var Version = "dev"
...
func Run(ctx context.Context, args []string, stdout, stderr io.Writer) error {
    ...
    if err := r.Command.Parse(args); err != nil {
        // Print help for whichever command was being parsed when the error occurred.
        // GetSelected returns the deepest successfully-selected command, so
        // "mycli shout -h" prints shout's help, not root's.
        selected := r.Command.GetSelected()
        if selected == nil {
            selected = r.Command
        }
        fmt.Fprintf(stderr, "\n%s\n", ffhelp.Command(selected))
    ...
    if err := r.Command.Run(ctx); err != nil {
        // Always print help for the selected command — both when a command has no
        // Exec (ErrNoExec: e.g. root invoked with no subcommand) and when a command
        // fails after being selected.
        fmt.Fprintf(stderr, "\n%s\n", ffhelp.Command(r.Command.GetSelected()))
        if errors.Is(err, ff.ErrNoExec) {
            return nil // not a failure; help was already printed
        }
        return err
    }
...
- `-h` / `--help` at any level causes `Parse` to return `ff.ErrHelp`; `main` treats it as success.
- A command with no `Exec` (e.g. root invoked without a subcommand) returns `ff.ErrNoExec`; the **dispatcher** prints help and returns nil — it never reaches `main`.
- Unknown subcommand returns an error; `main` owns the exit code.

### Entry Point — `main.go`

`ff.ErrHelp` is not a failure — treat it as success. `ff.ErrNoExec` is handled
in the dispatcher (help is printed, nil is returned) and never reaches `main`.
...
func TestShout(t *testing.T) {
    ...
    err := cmd.Run(context.Background(), []string{"shout", "hello", "world"}, &stdout, io.Discard)
    if err != nil {
...
func TestNoSubcommand(t *testing.T) {
    // Root with no subcommand: dispatcher prints help and returns nil.
    err := cmd.Run(context.Background(), []string{}, io.Discard, io.Discard)
    if err != nil {
...
| `-h` / `--help` | `flag.ErrHelp` returned from command; treated as success in `main` | `ff.ErrHelp` returned from `Parse`; treated as success in `main` | Handled by Cobra automatically at any level |
| Top-level help | `help` as special positional arg in dispatcher | `--help` / `-h` at any level via `ff` | `--help` / `-h` at any level |
| No subcommand given | Dispatcher prints command list; returns nil | Dispatcher prints root help via `ErrNoExec`; returns nil | Cobra prints root help; returns nil (when root has no `RunE`) |
...
| Testability | Lower — output not capturable without `os.Pipe` | Higher — `cmd.Run` accepts `io.Writer` | Higher — `Execute(ctx, args, stdout, stderr)` + per-command `cmd.ExecuteContext` |
